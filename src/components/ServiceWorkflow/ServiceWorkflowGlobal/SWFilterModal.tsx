...
const SWFilterModal = (props: AdvanceFilterModalProps) => {
  const { visible, filterObjInitial, onPressCancel, handleAdvanceSearch, fromCLP = false } = props;
  ...
  const fetchRequestTypeDropdownData = async (searchText: string = '') => {
    try {
      ...
      const requestTypeDropdownData = await ServiceWorkflowController.getServiceRequestTypeDropdownData(searchText);
      setRequestTypeDropdownData(requestTypeDropdownData)
      console.log('requestTypeDropdownData :>> ', requestTypeDropdownData);
    } catch (error) {
      console.log('error while fetching request type dropdown data :>> ', error);
      toast.error({
  ...
  const fetchAssignedToDropdownData = async (searchText: string = '') => {
    try {
      ...
      const assignedToDropdownData = await ServiceWorkflowController.getResponsiblePersonAndCreatorList(searchText);
            ...
            <ScrollView
              ...
              <View flex marginT-v5>
                ...
                <Dropdown
                  ...
                  searchQuery={fetchAssignedToDropdownData}
                  data={assignedToDropdownData}
                  value={
                    filterObj.assignedTo?.length > 0
                      ? filterObj.assignedTo[0].employeeNumber
                      : ''
                  }
                  keyboardAvoiding={true}
              ...
              <View marginT-v5 row centerV>
                <View flex marginR-v2>
                  ...
                  {createdDateErrorMsg ? <TextError errorMsg={createdDateErrorMsg} /> : null}
                </View>
              ...
              <View marginT-v5 row centerV>
                <View flex marginR-v2>
                  ...
                  {requestedDateErrorMsg ? <TextError errorMsg={requestedDateErrorMsg} /> : null}
                </View>
                <View flex>
                  {dropDownComponent({
                    title: 'Requested To',
